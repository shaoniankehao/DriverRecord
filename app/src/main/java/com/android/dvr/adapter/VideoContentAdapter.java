...
public class VideoContentAdapter extends RecyclerView.Adapter<VideoContentAdapter.ImageHolder> {
    private Context                        mContext;
    private ArrayList<VideoBean.VideoInfo> mData;

    public VideoContentAdapter(Context context, ArrayList<VideoBean.VideoInfo> arrayList) {
        LogUtils.e(getClass(), "PhotoContentAdapter");
        mContext = context;
        mData = arrayList;
    }
    ...
    public void onBindViewHolder(ImageHolder holder, final int position) {
        Glide.with(mContext).load(mData.get(position).filePath).into(holder.iv_picture);
        holder.iv_picture.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mOnItemClickListener != null) {
                    mOnItemClickListener.onItemClick(mData.get(position).filePath);
                }
            }
        });
        holder.iv_picture.setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (mOnItemClickListener != null) {
                    mOnItemClickListener.onItemLongClick(mData.get(position).filePath);
                }
                return true;
            }
        });
